    ...
    >
      ...
      <section className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="flex flex-col gap-4">
            ...
            <article key={item.id} className="rounded-[var(--radius-lg)] border border-[var(--color-border-bright)] bg-[var(--color-panel)] p-5 panel-elevated">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-[11px] uppercase tracking-[0.1em] text-[var(--color-text-tertiary)]">{item.code} / {item.category}</p>
                </div>
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--color-text-secondary)]">{item.summary}</p>
              <div className="mt-3 rounded-[var(--radius-md)] border border-[var(--color-border)] bg-white/3 px-4 py-3">
                <p className="text-[10px] uppercase tracking-[0.1em] text-[var(--color-text-tertiary)]">Execution Checklist</p>
          ...
          {data.items.length === 0 ? <p className="text-sm text-[var(--color-text-tertiary)]">No doctrine templates loaded.</p> : null}
        </div>

        <div className="flex flex-col gap-4">
          {canManageDoctrine ? (
            <CollapsiblePanel label="Create Doctrine" icon={<BookCheck size={16} className="text-[var(--color-accent)]" />}>
              <DoctrineCreateForm />
            </CollapsiblePanel>
          ) : (
