...
export function FleetDashboard({ isAdmin }: { isAdmin: boolean }) {
  ...
  async function handleSync() {
    ...
    setSyncResult(null);
    try {
      const res = await fetch("/api/admin/fleet/sync-specs", { method: "POST" });
  ...
  async function handleRemoveShip(shipId: string) {
    try {
      const res = await fetch(`/api/fleet/ships/${shipId}`, { method: "DELETE" });
...
function AddShipForm({ onAdded }: { onAdded: () => void }) {
  ...
  async function handleSearch(q: string) {
    ...
    setSearching(true);
    try {
      const res = await fetch(`/api/fleet/specs?q=${encodeURIComponent(q)}`);
      const data = await res.json();
      setSpecs(data.specs ?? []);
    } catch { setSpecs([]); }
    ...
    <div className="mb-3 rounded-[var(--radius-md)] border border-cyan-400/20 bg-cyan-400/5 p-3 space-y-2">
      {!selected ? (
        ...
        <div className="space-y-2">
          ...
          <input value={shipName} onChange={(e) => setShipName(e.target.value)} placeholder="Ship name (optional, e.g. \"Black Betty\")"
