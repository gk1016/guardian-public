...
function ChatPanel({ session, events }: { session: GuardianSession; events: FedEvent[] }) {
...
export function FederationPanel({ session }: { session: GuardianSession }) {
  ...
  // Fetch federation status and peers on mount + interval
  ...
  // Listen for federation events from WebSocket
  useEffect(() => {
    const unsub = subscribeAlerts((event: Record<string, unknown>) => {
      ...
      if (type?.startsWith("federation_")) {
        ...
        // Refresh peers list on connect/disconnect
