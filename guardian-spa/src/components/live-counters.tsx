...
export function LiveCounters() {
  ...
  const activeMissions = opsSummary?.active_missions ?? 0;
  const qrfReady = opsSummary?.qrf_ready ?? 0;
  const openRescues = opsSummary?.open_rescues ?? 0;
  const unreadAlerts = opsSummary?.unread_alerts ?? 0;
