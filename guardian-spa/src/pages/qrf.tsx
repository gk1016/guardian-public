            ...
            {canManage ? (
              <div className="mt-4 grid gap-3">
                <CollapsiblePanel label="Update posture" variant="inline">
                  <QrfStatusForm qrfId={item.id} initialAsset={{ status: item.status, platform: item.platform, locationName: item.locationName, availableCrew: item.availableCrew, notes: item.notes }} onSuccess={refetch} />
                </CollapsiblePanel>
                <CollapsiblePanel label="Dispatch asset" variant="inline">
                  <QrfDispatchForm qrfId={item.id} missionOptions={data?.missionOptions ?? []} rescueOptions={data?.rescueOptions ?? []} onSuccess={refetch} />
                </CollapsiblePanel>
              </div>
            ) : null}
                  ...
                  {item.dispatches.map((dispatch) => (
                    <div key={dispatch.id} className="rounded-[var(--radius-sm)] border border-[var(--color-border)] bg-white/3 px-3 py-2.5">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        {dispatch.targetHref ? (
                          <Link to={dispatch.targetHref} className="text-xs font-medium uppercase tracking-[0.1em] text-[var(--color-text-strong)] hover:text-cyan-200">{dispatch.targetLabel}</Link>
                        ) : (
                      ...
                      {canManage ? <div className="mt-2"><QrfDispatchStatusForm dispatchId={dispatch.id} initialStatus={dispatch.status} onSuccess={refetch} /></div> : null}
                    </div>
                  ))}
